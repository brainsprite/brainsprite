...
def transform_package(img_path,output_folder,overlay_path=''):
    ...
    if overlay_path == '':
        ...
        transform(img_path,output_folder+'bkg_mosaic.jpg',output_folder+'params.js',overlay_path,output_folder+'overlay_mosaic.png')

def transform(source_bkg_path,out_bkg_path,out_json,source_overlay_path='',out_overlay_path='',overlay_threshold=0.1):
    #load data
    bkg_vol = loadVolume(source_bkg_path)
    bkg_vol = (bkg_vol/bkg_vol.max())*255.
...
def test_mosaic():
    ...
    # transform data
    transform(output_folder+background,output_folder+'bkg_mosaic.jpg',output_folder+'params.json',output_folder+overlay,output_folder+'overlay_mosaic.png',overlay_threshold=0.3)
