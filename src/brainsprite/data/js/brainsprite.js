...
function initBrain (params) {
  ...
  const defaultParams = {
    ...
    showLR: true,
    currentOverlay: 0,
    overlayKeys: { prev: 'ArrowLeft', next: 'ArrowRight' }
  }
...
function initOverlay (overlayParams, nbSlice) {
  // Initialize a single overlay object - for internal use
  const overlay = Object.assign({}, overlayParams)
  overlay.opacity = typeof overlay.opacity !== 'undefined' ? overlay.opacity : 1
  overlay.sprite = document.getElementById(overlay.sprite)
  overlay.nbCol = overlay.sprite.width / nbSlice.Y
  overlay.nbRow = overlay.sprite.height / nbSlice.Z
  overlay.nbSlice = {
    X: typeof nbSlice.X !== 'undefined'
      ? nbSlice.X
      : overlay.nbCol * overlay.nbRow,
    Y: nbSlice.Y,
    Z: nbSlice.Z
  }
  return overlay
...
function brainsprite (params) { // eslint-disable-line no-unused-vars
  // Backward compat: wrap single overlay in overlays array
  if (params.overlay && !params.overlays) {
    params.overlays = [params.overlay]
  }
  ...
  if (params.overlays) {
    for (let i = 0; i < params.overlays.length; i++) {
      const ov = params.overlays[i]
      const initialized = initOverlay(ov, ov.nbSlice)
      if (ov.colorMap) {
        initialized.colorMap = initColorMap(ov.colorMap)
      }
      brain.overlays.push(initialized)
    }
  }
  brain.currentOverlay = typeof params.currentOverlay !== 'undefined' ? params.currentOverlay : 0
  brain.overlayKeys = params.overlayKeys || { prev: 'ArrowLeft', next: 'ArrowRight' }
  ...
  const updateValue = function () {
    // Update voxel value
    const pos = {}
    const activeOverlay = brain.overlays && brain.overlays.length > 0
      ? brain.overlays[brain.currentOverlay]
      : null
    if (activeOverlay && !brain.nanValue) {
      try {
        ...
        if (rgb[3] === 0) {
          ...
          const colorMap = activeOverlay.colorMap || brain.colorMap
          brain.voxelValue = getValue(rgb, colorMap)
        }
      } catch (err) {
  ...
  brain.init = function () {
    ...
    brain.planes.contextMaster.drawImage(brain.sprite,
      ...
      0, 0, brain.sprite.width, brain.sprite.height)
    if (brain.overlays && brain.overlays.length > 0) {
      // Draw the active overlay on the master canvas
      const ov = brain.overlays[brain.currentOverlay]
      brain.planes.contextMaster.globalAlpha = ov.opacity
      brain.planes.contextMaster.drawImage(ov.sprite,
        0, 0, ov.sprite.width, ov.sprite.height,
        0, 0, brain.sprite.width, brain.sprite.height)
    };
  ...
  if (brain.overlays) {
    for (let i = 0; i < brain.overlays.length; i++) {
      brain.overlays[i].sprite.addEventListener('load', function () {
        brain.init()
        brain.drawAll()
      })
    }
  ...
  // Switch to a different overlay by index
  brain.setCurrentOverlay = function (index) {
    if (!brain.overlays || index < 0 || index >= brain.overlays.length) return
    brain.currentOverlay = index
    // Swap to per-overlay colormap if present, otherwise keep global
    if (brain.overlays[index].colorMap) {
      brain.colorMap = brain.overlays[index].colorMap
    }
    brain.init()
    brain.drawAll()
  }

  // Keyboard navigation between overlays
  if (brain.overlays && brain.overlays.length > 1) {
    document.addEventListener('keydown', function (e) {
      if (e.key === brain.overlayKeys.next) {
        brain.setCurrentOverlay(brain.currentOverlay + 1)
      } else if (e.key === brain.overlayKeys.prev) {
        brain.setCurrentOverlay(brain.currentOverlay - 1)
      }
    })
  }
